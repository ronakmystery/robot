...
# --- Object size filter (NEW) ---
MIN_OBJ_SIZE  = 50     # ignore tiny red noise
MAX_OBJ_SIZE  = 200    # ignore huge close blobs

# --- Yellow HSV range ---
# --- Broad Yellow HSV range (handles dull / faded yellow) ---
COLOR_RANGES = [
    ([15, 60, 60], [45, 255, 255])
]


print("🎥 Tracking yellow ball — press 'q' to quit")
while True:
    ret, frame = cap.read()
    if not ret:
        continue
    ...
    if contours:
        ...
        # --- Skip if object outside valid size range ---
        if h < MIN_OBJ_SIZE or h > MAX_OBJ_SIZE:
            cv2.putText(frame, "Invalid size", (10, 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
            action = {"mode":"stop","walk_type":"flat","backwards":False,
                      "lateral":0,"height":0,"pitch":0,"roll":0,
                      "speed":0,"pause":False,"start":False}
            send_action(action)
        else:
            ...
            cv2.putText(frame, f"size={h} speed={speed} back={backwards}",
